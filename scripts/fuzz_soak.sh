...
seconds_per_target="${ENVQ_FUZZ_SECONDS:-900}"
targets_raw="${ENVQ_FUZZ_TARGETS:-parse_roundtrip edit_set_unset list_output diff}"
extra_args_raw="${ENVQ_FUZZ_ARGS:-}"
cleanup_on_success="${ENVQ_FUZZ_CLEANUP:-0}"

read -r -a targets <<< "$targets_raw"
read -r -a extra_args <<< "$extra_args_raw"

if ! [[ "$seconds_per_target" =~ ^[0-9]+$ ]] || [[ "$seconds_per_target" -le 0 ]]; then
  echo "error: ENVQ_FUZZ_SECONDS must be a positive integer, got '$seconds_per_target'" >&2
  exit 1
fi
...
for target in "${targets[@]}"; do
  case "$target" in
    parse_roundtrip|edit_set_unset|list_output|diff) ;;
    *)
      echo "error: unsupported fuzz target '$target'" >&2
      echo "supported targets: parse_roundtrip edit_set_unset list_output diff" >&2
      exit 1
      ;;
  esac
done

tmp_root="$(mktemp -d /tmp/envq_fuzz_soak.XXXXXX)"
success=0
